...
class ResidualBlock(nn.Module):
    ...
    def forward(self,x, fmp_dict = None, prefix = ""):
        out = self.conv1(x)
        out = self.bn1(out)
        out = torch.relu(out)
        out = self.conv2(out)
        ...
        if fmp_dict is not None:
            fmp_dict[f"{prefix}.conv"] = out_add

        out = torch.relu(out_add)
        if fmp_dict is not None:
            fmp_dict[f"{prefix}.relu"] = out
        return out
    ...
    def forward(self,x, return_features_maps=False):
        if not return_features_maps:
            ...
            for i, block in enumerate(self.layer1):
                x=block(x, feature_maps, prefix=f"layer1.{i}")
            feature_maps['layer1'] = x
            for i, block in enumerate(self.layer2):
                x=block(x, feature_maps, prefix=f"layer2.{i}")
            feature_maps['layer2'] = x
            for i, block in enumerate(self.layer3):
                x=block(x, feature_maps, prefix=f"layer3.{i}")
            feature_maps['layer3'] = x
            for i, block in enumerate(self.layer4):
                x=block(x, feature_maps, prefix=f"layer4.{i}")
